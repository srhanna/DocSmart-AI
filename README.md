...
- **`pages/api/upload.js`** — Next.js API route (`POST /api/upload`) that receives the uploaded file using `formidable`, saves it to an `uploads/` directory on the server, and returns file metadata (name, path, size, MIME type).
